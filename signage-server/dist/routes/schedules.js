...
exports.setupScheduleRoutes = setupScheduleRoutes;
const index_1 = require("../index");
const uuid_1 = require("uuid");
function setupScheduleRoutes(app) {
    // List all schedules
    ...
    app.post('/api/schedules', async (req, res) => {
        const { screenId, playlistId, startTime, endTime, daysOfWeek, startDate, endDate, priority, isActive } = req.body;
      ...
      `, [
            ...
            (0, index_1.broadcastUpdate)({ type: 'schedule_created', schedule });
    ...
    app.put('/api/schedules/:id', async (req, res) => {
      ...
      `, [id, screenId, playlistId, startTime, endTime, daysOfWeek, startDate, endDate, priority, isActive]);
            ...
            const schedule = result.rows[0];
            (0, index_1.broadcastUpdate)({ type: 'schedule_updated', schedule });
            res.json(schedule);
        }
        catch (error) {
    ...
    app.delete('/api/schedules/:id', async (req, res) => {
        const { id } = req.params;
        try {
            const result = await index_1.pool.query('DELETE FROM schedules WHERE id = $1 RETURNING id', [id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Schedule not found' });
            }
            (0, index_1.broadcastUpdate)({ type: 'schedule_deleted', scheduleId: id });
