...
exports.default = setupProxyRoutes;
const express_1 = require("express");
const bx_session_1 = require("../lib/bx-session");
const router = (0, express_1.Router)();
// Fetch BX data from BX API with authentication
async function fetchBXData() {
    try {
        // Get server-side BxSoftware session
        const bxSid = await (0, bx_session_1.getBxSession)();
        if (!bxSid) {
        ...
        console.log('✅ BX data fetched successfully:', data.totalcount || 0, 'items');
...
function setupProxyRoutes(pool) {
    // API endpoint to get BX data (for frontend)
    router.get('/bx-data', async (req, res) => {
        try {
            ...
            if (Array.isArray(data)) {
                data.forEach((list) => {
                    ...
                    if (Array.isArray(list.lines)) {
                        list.lines.forEach((line) => totalItems += line.quantity || 0);
                    }
                });
            }
            res.json({
    ...
    router.get('/logistic-dashboard-embed', async (req, res) => {
        try {
            const orientation = req.query.orientation || 'landscape';
            const layout = req.query.layout || (orientation === 'portrait' ? 'vertical' : 'horizontal');
            const refresh = parseInt(req.query.refresh) || 30; // Refresh interval in seconds
