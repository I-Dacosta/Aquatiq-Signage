...
exports.setupScreenAPIRoutes = setupScreenAPIRoutes;
const index_1 = require("../index");
const scheduler_1 = require("../services/scheduler");
const monitor_1 = require("../services/monitor");
const schedule_resolver_1 = require("../lib/schedule-resolver");
function setupScreenAPIRoutes(app) {
    // Enhanced endpoint for URL Launcher with schedule resolution
    // Screen requests: GET /api/signage/screen/{screen-id}
    app.get('/api/signage/screen/:screenId', async (req, res) => {
      ...
      `, [screenId, req.ip]);
            ...
            const playlistId = await (0, schedule_resolver_1.getActivePlaylist)(index_1.pool, screenId);
            if (!playlistId) {
            ...
            const currentSchedule = await (0, schedule_resolver_1.getCurrentSchedule)(index_1.pool, screenId);
            // Get playlist content
            const content = await (0, scheduler_1.getPlaylistContent)(index_1.pool, playlistId);
            if (content.length === 0) {
                return res.status(404).json({
            ...
            const currentPosition = parseInt(position) || 0;
      ...
      `, [screenId, currentContent.id]);
            // Log status
            await (0, monitor_1.logScreenStatus)(index_1.pool, screenId, 'playing', currentContent.id);
    ...
    app.post('/api/signage/screen/:screenId/heartbeat', async (req, res) => {
        const { screenId } = req.params;
      ...
      `, [screenId, contentId]);
            if (status) {
                await (0, monitor_1.logScreenStatus)(index_1.pool, screenId, status, contentId, errorMessage);
            }
    ...
    app.get('/api/signage/screen/:screenId/info', async (req, res) => {
        const { screenId } = req.params;
        try {
    ...
    app.get('/api/screen-api/:mac/current', async (req, res) => {
        const { mac } = req.params;
        try {
            // Find screen by MAC address
            const screenResult = await index_1.pool.query('SELECT id FROM screens WHERE mac_address = $1', [mac]);
            ...
            await index_1.pool.query('UPDATE screens SET last_seen = NOW(), is_online = true WHERE id = $1', [screenId]);
            res.json({
    ...
    app.post('/api/screen-api/:mac/heartbeat', async (req, res) => {
        const { mac } = req.params;
      ...
      `, [mac, ip_address, content_id]);
            ...
            // Log heartbeat
            await (0, monitor_1.logScreenStatus)(index_1.pool, result.rows[0].id, 'online', content_id);
            res.json({ success: true, timestamp: new Date().toISOString() });
        }
        catch (error) {
            console.error('Error handling heartbeat:', error);
