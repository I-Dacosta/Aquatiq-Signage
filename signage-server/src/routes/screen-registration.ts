...
export function setupScreenRegistrationRoutes(app: Router, pool: Pool) {
  /**
   ...
   */
  app.post('/api/screen-api/register', async (req: Request, res: Response) => {
    try {
      const {
        mac_address,
        name = 'Ny Skjerm',
      ...
      const existingScreen = await pool.query(
        'SELECT * FROM screens WHERE mac_address = $1',
        [mac_address]
      );
      ...
      const defaultPlaylist = await pool.query(
   ...
   */
  app.get('/api/screen-api/setup/:setupId', async (req: Request, res: Response) => {
    try {
 ...
 */
async function createDefaultPlaylist(pool: Pool): Promise<void> {
  try {
    // Create default playlist
    const playlist = await pool.query(
      `INSERT INTO playlists (name, description, is_default)
    ...
    const welcomeContent = await pool.query(
      `INSERT INTO content (name, type, url, duration, is_active)
       VALUES ('Velkommen', 'url', $1, 10, true)
       RETURNING id`,
      [`${process.env.VIDEO_BASE_URL || 'https://signage.aquatiq.com'}/offline.html`]
    );
    ...
    await pool.query(
      `INSERT INTO playlist_items (playlist_id, content_id, order_index)
       VALUES ($1, $2, 1)`,
      [playlistId, welcomeContent.rows[0].id]
    );
