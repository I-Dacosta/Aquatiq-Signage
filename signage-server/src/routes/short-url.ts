...
export function setupShortUrlRoutes(app: Router, pool: Pool) {
  /**
  ...
  app.post('/api/short-url', async (req: Request, res: Response) => {
    try {
      ...
      // Store in database
      await pool.query(
        `INSERT INTO short_urls (short_id, name, location, template, created_at, expires_at)
         VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP + INTERVAL '1 year')`,
        [shortId, name, location, template]
      );
   ...
   */
  app.get('/tv/:shortId', async (req: Request, res: Response) => {
    try {
      const { shortId } = req.params;

      // Look up short URL in database
      const result = await pool.query(
        `SELECT name, location, template, expires_at 
         FROM short_urls 
         WHERE short_id = $1`,
        [shortId]
      );
   ...
   */
  app.get('/api/short-url/:shortId', async (req: Request, res: Response) => {
    try {
      const { shortId } = req.params;

      const result = await pool.query(
        'SELECT * FROM short_urls WHERE short_id = $1',
        [shortId]
      );
