...
export async function applyTemplate(
  ...
  for (const screen of template.screens) {
    const result = await pool.query(
      `INSERT INTO screens (name, location, group_name, mac_address)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [screen.name, screen.location, screen.group_name, screen.mac_address || null]
    );
  ...
  for (const content of template.content) {
    const result = await pool.query(
      `INSERT INTO content (name, type, url, duration, metadata, is_active)
       VALUES ($1, $2, $3, $4, $5, true)
       RETURNING *`,
      [content.name, content.type, content.url, content.duration, JSON.stringify(content.metadata || {})]
    );
  ...
  for (const playlist of template.playlists) {
    const result = await pool.query(
      `INSERT INTO playlists (name, description, is_default)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [playlist.name, playlist.description, playlist.is_default]
    );
    ...
    for (let i = 0; i < results.content.length; i++) {
      await pool.query(
        `INSERT INTO playlist_items (playlist_id, content_id, order_index)
         VALUES ($1, $2, $3)`,
        [playlist.id, results.content[i].id, i]
      );
    }
  }
