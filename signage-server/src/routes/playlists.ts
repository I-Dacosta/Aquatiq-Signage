...
export function setupPlaylistRoutes(app: Express) {
  // List all playlists
  ...
  app.post('/api/playlists', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (isDefault) {
        await pool.query('UPDATE playlists SET is_default = false');
      }
      ...
      broadcastUpdate({ type: 'playlist_created', playlist });
  ...
  app.put('/api/playlists/:id', async (req: Request, res: Response) => {
    ...
    try {
      if (isDefault) {
        await pool.query('UPDATE playlists SET is_default = false WHERE id != $1', [id]);
      }
      ...
      const playlist = result.rows[0];
      broadcastUpdate({ type: 'playlist_updated', playlist });
  ...
  app.delete('/api/playlists/:id', async (req: Request, res: Response) => {
    ...
    try {
      ...
      broadcastUpdate({ type: 'playlist_deleted', playlistId: id });
    ...
    try {
      ...
      if (order === undefined) {
        const maxResult = await pool.query(
          'SELECT COALESCE(MAX(order_index), -1) + 1 as next_order FROM playlist_items WHERE playlist_id = $1',
          [id]
        );
      ...
      broadcastUpdate({ type: 'playlist_item_added', playlistId: id, item: result.rows[0] });
  ...
  app.delete('/api/playlists/:id/items/:itemId', async (req: Request, res: Response) => {
    ...
    try {
      ...
      broadcastUpdate({ type: 'playlist_item_removed', playlistId: id, itemId });
  ...
  app.put('/api/playlists/:id/reorder', async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    try {
      ...
      broadcastUpdate({ type: 'playlist_reordered', playlistId: id });
